...
Environment variables (all optional):
  ...
  CLAUDE_MODEL                  Primary model  (default: claude-sonnet-4-5)
  CLAUDE_FALLBACK_MODELS        Comma-separated fallback models tried in order
  MAX_LLM_CALLS                 Hard cap on API calls per run  (default: 30)
  ALLOW_DETERMINISTIC_FALLBACK  Set to 1 to fall back to rule-based planner on
                                  quota exhaustion  (default: 0 — LLM only)
  CLAUDE_MAX_RETRIES            Per-call retry attempts on rate-limit  (default: 3)
"""
...
api_key = "REDACTED_ANTHROPIC_API_KEY"  # YOUR API KEY HERE
...
class Agent:
    ...
    Robustness features (mirroring the Gemini implementation):
      - Per-call retry with exponential back-off on rate-limit / overload errors
      - Ordered fallback model list (Claude is stateless, so model switches are
        safe at any point in the conversation)
      - Optional deterministic planner as last-resort fallback when
        allow_deterministic_fallback=True
      - Hard cap on LLM calls via max_llm_calls
    """
    ...
    def __init__(
        ...
        # Build ordered candidate list; deduplication preserves insertion order.
        self.model_candidates = list(dict.fromkeys([model_name] + (fallback_models or [])))
        self.active_model_index = 0

        if not api_key:
            if self.allow_deterministic_fallback:
                print("[Agent] ANTHROPIC_API_KEY not set. Falling back to deterministic planner.")
                self._use_llm = False
            else:
                raise RuntimeError(
                    "ANTHROPIC_API_KEY is not set. "
                    "Export it or set ALLOW_DETERMINISTIC_FALLBACK=1 to use the rule-based planner."
                )
        else:
            self._use_llm = True
    ...
    def _is_retryable_error(exc: Exception) -> bool:
        """True for rate-limit, overload, and transient connection errors."""
        if isinstance(exc, (anthropic.RateLimitError, anthropic.APIConnectionError)):
            return True
        if isinstance(exc, anthropic.APIStatusError):
            # 529 = Anthropic overloaded; 429 = rate limit
            return exc.status_code in (429, 529)
        return False

    @staticmethod
    def _retry_delay_seconds(exc: Exception) -> int:
        """Parse retry-after from the error message, or fall back to 5 s."""
        msg = str(exc)
        # Anthropic sometimes embeds 'retry-after: N' or 'retry in Ns' in the body
        match = re.search(r"retry[- ](?:after|in)\s*:?\s*([0-9]+(?:\.[0-9]+)?)", msg, re.IGNORECASE)
        if match:
    ...
    def _switch_to_next_model(self) -> bool:
        ...
        Unlike Gemini's stateful chat, Claude is stateless — we pass the full
        message history on every call, so a mid-run model switch is always safe.
        Returns True if a new model was selected, False if the list is exhausted.
        """
        if self.active_model_index + 1 >= len(self.model_candidates):
            return False
        self.active_model_index += 1
        self.model_name = self.model_candidates[self.active_model_index]
        print(f"[Agent] Switching to fallback LLM model: {self.model_name}")
        return True
    ...
    def _call_with_retries(self, messages: list[dict]) -> Any:
        """
        Call the Claude API with retry logic for rate-limit / overload errors.
        Tries each model in model_candidates before giving up.
        """
        last_exc: Exception | None = None

        # Outer loop: one pass per model candidate
        for model_idx in range(len(self.model_candidates)):
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    print(f"[Agent] API error ({type(exc).__name__}): {str(exc)[:200]}")

                    if attempt < self.max_retries:
                        print(f"[Agent] Retrying in {delay}s "
                              f"(attempt {attempt}/{self.max_retries}, model: {self.model_name}).")
                        time.sleep(delay)
                    else:
                        # Exhausted retries for this model; try the next one
                        if self._switch_to_next_model():
                            print(f"[Agent] All retries exhausted for previous model. "
                                  f"Trying {self.model_name}.")
                            break  # Break inner loop → advance outer loop
                        else:
                            raise RuntimeError(
                                f"All models and retries exhausted. Last error: {last_exc}"
                            ) from last_exc
    ...
    # ── Deterministic helpers (fallback planner) ──────────────────────────

    def _select_hospital(self, incident_node: str) -> str | None:
        """Return the nearest hospital with available capacity."""
    ...
    def _select_vehicle(self, incident_id: str) -> tuple[str | None, str | None]:
        """Return (vehicle_id, hospital_node) for the best available vehicle."""
        inc = self.world.incidents[incident_id]
        best_vehicle, best_cost = None, math.inf

        for vehicle_id, v in self.world.vehicles.items():
            if not v["available"]:
                continue
            if set(inc["required_capabilities"]) - set(v["capabilities"]):
                continue
            if inc["patients"] > 0 and "patient_transport" in v["capabilities"]:
                if inc["patients"] > v["capacity"] - v["current_load"]:
                    continue
            cost, _ = self.world.dijkstra(v["location"], inc["location"])
            if cost < best_cost:
                best_cost, best_vehicle = cost, vehicle_id

        if best_vehicle is None:
            return None, None

        hospital_node = None
        v = self.world.vehicles[best_vehicle]
        if inc["patients"] > 0 and "patient_transport" in v["capabilities"]:
    ...
    def _run_deterministic(self, start_step: int = 0) -> dict:
        """Rule-based planner used when the LLM is unavailable or quota is hit."""
        ...
        while local_steps < self.max_steps:
            ...
            for incident_id, _ in ranked:
                ...
                local_steps += 1
                print(f"[Deterministic] Dispatch {vehicle_id} -> {incident_id} | "
        ...
        while self.step_count < self.max_steps:
            ...
            if llm_calls >= self.max_llm_calls:
                ...
                raise RuntimeError(
                    ...
                    "Increase MAX_LLM_CALLS or set ALLOW_DETERMINISTIC_FALLBACK=1."
                )

            # ── Call Claude (with retry + model fallback) ─────────────────
            try:
                response = self._call_with_retries(messages)
                llm_calls += 1
            except Exception as exc:
                if self.allow_deterministic_fallback and self._is_retryable_error(exc):
                    print(f"[Agent] API call failed: {exc}\n"
                          "[Agent] Switching to deterministic planner.")
                    return self._run_deterministic(start_step=self.step_count)
                raise
...
if __name__ == "__main__":
    model_name_env = os.getenv("CLAUDE_MODEL", "claude-sonnet-4-5").strip()
    fallback_models_env = [
    ...
    max_retries_env            = int(os.getenv("CLAUDE_MAX_RETRIES", "3"))
